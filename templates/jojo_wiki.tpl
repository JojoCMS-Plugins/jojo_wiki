{if $wiki_can_edit}
    <div id="wiki-edit-bar"{if !$wiki.wikiid} class="create-new-wiki"{/if}>
    ...
    {if !$wiki.wikiid}
        <div class="create-new-wiki">Create a new wiki entry</div>
    {/if}
{/if}
...
{if $wiki_can_edit}
    <div id="wiki-edit"{if $wiki.wikiid} style="display: none"{/if}>
        <form method="post" action="actions/wiki_save.php?arg1={$wiki.wikiid}" target="frajax-iframe">
            ...
            <div{if $wiki} class="create-new-wiki"{/if}>{if !$wiki.wikiid}URL: {/if}<input type="{if !$wiki.wikiid}text{else}hidden{/if}" name="url" value="{$wiki.wk_url}" /></div>
